...
const trustPoints = [
  {
    ...
    description: "Tu peux comparer sans souscrire et sans frais cachés.",
    icon: "check" as const,
  },
  {
    title: "Méthode lisible",
    description: "On sépare meilleur choix, meilleur prix, cashback et sponsorisation.",
    icon: "search" as const,
  },
  {
    title: "Données protégées",
    description: "Les informations servent à la comparaison, pas à brouiller ton choix.",
    icon: "lock" as const,
  },
];
...
export default function Home() {
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      {/* Navigation */}
      <section className="sticky top-0 z-40 border-b border-white/10 bg-slate-950/95 backdrop-blur-2xl">
        <div className="mx-auto max-w-7xl px-5 py-4 sm:px-8 lg:px-10">
          <div className="rounded-full border border-white/5 bg-white/[0.02] px-4 py-3 backdrop-blur">
            <SiteNav />
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <Link
                href="/comparateurs"
                className="rounded-lg bg-gradient-to-r from-cyan-500 to-blue-500 px-10 py-4 text-center font-bold text-white shadow-2xl shadow-cyan-500/40 transition hover:shadow-cyan-500/60 hover:from-cyan-400 hover:to-blue-400"
              >
                Comparer maintenant →
              </Link>
              <a
                href="#univers-comparia"
                className="rounded-lg border border-white/20 bg-white/5 px-8 py-4 text-center font-semibold text-white transition hover:bg-white/10"
              >
                Voir les univers
              </a>
            </div>
              ...
              <div className="mt-5 max-h-[22rem] space-y-3 overflow-y-auto pr-2">
                {pillars.map((category, index) => (
                  <Link
                    ...
                    className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] p-4 transition hover:border-cyan-300/30 hover:bg-cyan-400/10"
                  >
                    <span className="flex items-center gap-3">
                      ...
                      <span>
                        ...
                        <span className="mt-1 block text-sm text-slate-400">{category.group}</span>
                      </span>
                    </span>
      ...
      <section className="px-5 pb-14 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl rounded-[2rem] border border-cyan-300/15 bg-cyan-300/[0.06] p-6 sm:p-8">
          <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-cyan-300">Confiance partenaires</p>
              <h2 className="mt-3 text-3xl font-bold sm:text-4xl">Un tunnel propre pour des annonceurs sérieux.</h2>
              <p className="mt-4 text-sm leading-7 text-slate-300">
                Comparia est pensé pour l’affiliation long terme : demandes qualifiées, consentement, pages légales, transparence et suivi des conversions.
              </p>
              <Link
                href="/transparence-affiliation"
                className="mt-5 inline-flex rounded-full border border-cyan-300/30 bg-cyan-300/10 px-5 py-3 text-sm font-semibold text-cyan-100 transition hover:bg-cyan-300/15"
              >
                Voir la transparence affiliation
              </Link>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {partnerSignals.map(([title, description]) => (
                <article key={title} className="rounded-2xl border border-white/10 bg-slate-950/60 p-5">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            {[
              ...
              <article key={item.title} className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.04]">
                ...
                <div className="p-5">
                  <div className="flex items-center gap-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-cyan-400/10 text-cyan-300 ring-1 ring-inset ring-cyan-300/20">
                      <CompariaIcon name={item.icon} className="h-5 w-5" />
                    </span>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {[
              ...
              <div
                key={title}
                className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.05] to-white/[0.02] p-6 hover:border-cyan-400/30 transition"
              >
      ...
      <section className="px-5 pb-14 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-4xl rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 sm:p-8">
          ...
          <div className="mt-6 space-y-4">
            {faqs.map((faq) => (
              <article key={faq.question} className="rounded-2xl border border-white/10 bg-slate-950/60 p-5">
      ...
      <section className="relative px-5 pb-14 sm:px-8 lg:px-10">
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          <Link
            href="/comparateurs"
            className="inline-block rounded-lg bg-gradient-to-r from-cyan-500 to-blue-500 px-12 py-4 font-bold text-lg text-white shadow-2xl shadow-cyan-500/40 transition hover:shadow-cyan-500/60"
          >
            Comparer maintenant →
          </Link>
        </div>
