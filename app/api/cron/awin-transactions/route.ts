...
export async function GET(request: Request) {
  const limited = rateLimit(request, { key: "cron-awin-transactions", limit: 8, windowMs: 60 * 60 * 1000 });
  ...
  try {
    const summary = await syncAwinTransactions({ days: 31 });
    return secureJson({
      ok: true,
      job: "awin_transactions_sync",
...
function sanitizeErrorMessage(message: string) {
  const secrets = [process.env.AWIN_API_TOKEN, process.env.SUPABASE_SERVICE_ROLE_KEY, process.env.ADMIN_DASHBOARD_TOKEN, process.env.CRON_SECRET].filter(
